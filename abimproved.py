...
blacklisted_leads = "^(walther GSP|PM63 RAK SEMI PM 63|.*[Uu]chwyt.*|[ŁłLl]adownica.*|[Kk]abura.*|[Kk]upi.*|[Ll]atarka.*|[Aa]municj.*|[Kk]olb.*|[Mm]agazyne.*|.*DOWOZIMY.*|.*[Łł]uski.*|AMUNICJA.*|Chwyt.*|S[lł]uchawki.*|.*ulary.*)$"
...
class Adv:
	...
	def __str__(self):
		...
		if self.advtype != "Sprzedaż" or self.price == "Do uzgodnienia" or self.advtype == "" or re.match(re.compile(blacklisted_leads), self.lead) != None or re.match(re.compile(blacklisted_text), self.text) != None:
			return ""
